...
removing static obstacles, indeed these obstacles were taken into
...
account such obstacles.%(\textcolor{red}{TODO: pourquoi?}).
Also thepoints (representing people) were not able to avoid packed
places anymore and just went strait to there goal, same as for obstacles,
packed places
were taken into account by the edges of the graph which weigth
...
a $d\theta$ and minimizing the objective function (either a graph-based
distance or the euclidean distance) on those angles. 
